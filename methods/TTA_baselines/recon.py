...
class ReconHead(nn.Module):
    """
    Lightweight reconstruction head for self-supervised TTA.
    Upsamples encoder features back to input image resolution.
    """
    def __init__(self, encoder_dim=1024, hidden_dim=256, out_channels=1):
        super().__init__()
        self.head = nn.Sequential(
            nn.Conv2d(encoder_dim, hidden_dim, kernel_size=1),
            nn.GroupNorm(32, hidden_dim),
            nn.ReLU(inplace=True),
...
class SupervisedTrainerWithSegTTT:
    ...
    def __init__(self, frozen_segnet, cfg):
        ...
        self.seghead_optimizer = torch.optim.Adam(
            self.ttt_seg_head.parameters(),
            lr=self.inner_lr
        )
        if recon:
            self.ttt_recon_head = ReconHead(
                encoder_dim=cfg.get('encoder_dim', 1024),
                out_channels=1  # single channel B-mode image
            ).to(self.device)

            self.recon_head_optimizer = torch.optim.Adam(
                self.ttt_recon_head.parameters(),
                lr=self.inner_lr
            )
    ...
    def _build_seghead(self):
        ...
        seghead = self.cfg.get('seghead', None)
        if seghead == 'linear_probe':
            return LinearProbeSegHead(encoder_dim).to(self.device)
        elif seghead == 'light':
            return LightSegHead(encoder_dim, 256).to(self.device)
        else:
            return SegHead(encoder_dim, 256).to(self.device)
    
    def _build_reconhead(self):
        return ReconHead(encoder_dim=self.cfg.get('encoder_dim', 1024),out_channels=1).to(self.device)
    ...
    def reset_objects(self):
        ...
        if not self.stateful_head:
            ...
            elif hasattr(self, 'ttt_recon_head'):
                self.ttt_recon_head = self._build_reconhead()
                self.recon_head_optimizer = torch.optim.Adam(
                    self.ttt_recon_head.parameters(),
                    lr=self.inner_lr
                )
    ...
    def apply_reconstruction_ttt(self, model, batch, val_iter):

        model.train()
        self.ttt_recon_head.train()
        ...
        for step in range(self.inner_steps):
            ...
            encoder_optimizer.zero_grad()
            self.recon_head_optimizer.zero_grad()
            ...
            # Reconstruct input image from encoder features
            recon = self.ttt_recon_head(feats)
            recon = F.interpolate(
                recon,
                size=image.shape[2:],
                mode='bilinear',
            ...
            loss = F.l1_loss(recon, image) + F.mse_loss(recon, image)
            recon_losses.append(loss.item())

            loss.backward()
            torch.nn.utils.clip_grad_norm_(encoder_params, 1.0)
            torch.nn.utils.clip_grad_norm_(
                self.ttt_recon_head.parameters(), 1.0
            )
            encoder_optimizer.step()
            self.recon_head_optimizer.step()

        wandb.log({
            "ttt/recon_loss_mean": np.mean(recon_losses),
            "ttt/recon_loss_delta": recon_losses[-1] - recon_losses[0],
            "ttt/epoch": val_iter,
        })
