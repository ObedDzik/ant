...
def main(cfg):
    ...
    if cfg.base_run_test:
        ...
        if cfg.use_antta:
            ...
            test_metrics, results_table = run_test_epoch(
...
def run_train_epoch(
    ...
    for train_iter, data in enumerate(tqdm(loader, desc=desc)):
        ...
        if args.debug_train and train_iter > 10:
            break
...
def run_test_epoch(args, model, loader, epoch, desc="test", log_fn=None, ttt_seg_object=None):
    model.eval()
    batch_size = args.data.batch_size
    evaluator = Evaluator(**args.evaluator)
    ttt_times_per_image = []

    for test_iter, data in enumerate(tqdm(loader, desc=desc)):
