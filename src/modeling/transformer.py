...
class JEPAPredictor(nn.Module):
    """
    JEPA-style predictor that transforms ViT embeddings to predict CNN embeddings.
    
    Architecture:
        Input [B, N, d_vit] → Bottleneck Projection → [B, N, bottleneck] →
        TransformerEncoder → [B, N, bottleneck] → Projection → [B, N, d_cnn]
    """
    ...
    def __init__(
        ...
        """
        Args:
            d_model_vit (int): ViT embedding dimension (e.g., 768)
            d_model_cnn (int): CNN embedding dimension (e.g., 512)
            bottleneck (int): Bottleneck dimension for transformer (e.g., 384)
                           This reduces model size by operating in a smaller space
            n_layers (int): Number of transformer encoder layers
            n_heads (int): Number of attention heads
            dropout (float): Dropout rate
            use_layernorm (bool): Whether to add LayerNorm after projection
        """
        super().__init__()
        
        self.d_model_vit = d_model_vit
        ...
        # Input projection: ViT dim → bottleneck dim
        self.input_projection = nn.Linear(d_model_vit, bottleneck)
        
        # Optional: LayerNorm after input projection
        if use_layernorm:
            self.input_norm = nn.LayerNorm(bottleneck)
        
        # Transformer encoder operates in smaller bottleneck dimension
        self.transformer = TransformerEncoder(
            n_layers=n_layers,
            n_heads=n_heads,
            d_model=bottleneck,  # Use bottleneck dimension
            d_feed_forward=bottleneck * 4,  # Standard 4x expansion
            dropout=dropout,
        )
        
        # Output projection: bottleneck dim → CNN dim
        self.output_projection = nn.Linear(bottleneck, d_model_cnn)
        
        # Optional: LayerNorm after output projection for stability
        if use_layernorm:
            self.output_norm = nn.LayerNorm(d_model_cnn)
    ...
    def forward(self, x, mask=None, return_attn=False):
        ...
        Args:
            x: [B, N, d_vit] ViT embeddings
            mask: Optional attention mask
            return_attn: Whether to return attention weights
        
        Returns:
            If return_attn=False:
                [B, N, d_cnn] projected embeddings
            If return_attn=True:
                ([B, N, d_cnn], attention_dict)
        """
        # Project to bottleneck dimension
        x = self.input_projection(x)  # [B, N, d_vit] -> [B, N, bottleneck]
        if self.use_layernorm:
            x = self.input_norm(x)
        
        # Pass through transformer encoder
        if return_attn:
            x, attn_dict = self.transformer(x, mask=mask, return_attn=True)
        else:
            x = self.transformer(x, mask=mask)
        # x: [B, N, bottleneck]
        
        # Project to CNN dimension
        x = self.output_projection(x)  # [B, N, bottleneck] -> [B, N, d_cnn]
    ...
    def get_intermediate_layers(self, x, mask=None, n=1):
        """
        Get outputs from intermediate transformer layers.
        
        Args:
            x: [B, N, d_vit] input embeddings
            mask: Optional attention mask
            n: Number of last layers to return (or list of layer indices)
        
        Returns:
            List of [B, N, d_cnn] tensors from specified layers
        """
        # Project to bottleneck dimension
        ...
        _, attn_dict = self.transformer(x, mask=mask, return_attn=True)
        all_layer_outputs = attn_dict['layer_outputs']
        
        # Select which layers to return
        if isinstance(n, int):
            # Return last n layers
            selected_layers = all_layer_outputs[-n:]
        else:
            # Return specific layer indices
            selected_layers = [all_layer_outputs[i] for i in n]
        
        # Project each layer output to CNN dimension
        for layer_out in selected_layers:
            projected = self.output_projection(layer_out)
            if self.use_layernorm:
...
class JEPAPredictornb(nn.Module):
    """
    JEPA-style predictor that transforms ViT embeddings to predict CNN embeddings.
    
    Architecture:
        Input [B, N, d_vit] → TransformerEncoder → [B, N, d_vit] → Projection → [B, N, d_cnn]
    """
    ...
    def __init__(
        ...
        # Transformer encoder operates in ViT dimension
        self.transformer = TransformerEncoder(
            n_layers=n_layers,
            n_heads=n_heads,
            d_model=d_model_vit,
            d_feed_forward=d_model_vit * 4,  # Standard 4x expansion
            dropout=dropout,
        )
        
        # Project from ViT dimension to CNN dimension
        self.projection = nn.Linear(d_model_vit, d_model_cnn)
        
        # Optional: LayerNorm after projection for stability
        if use_layernorm:
            self.proj_norm = nn.LayerNorm(d_model_cnn)
        
        self._init_weights()
    
    def _init_weights(self):
        """Initialize projection weights."""
        # Xavier/Glorot initialization for projection
    ...
    def forward(self, x, mask=None, return_attn=False):
        ...
        # Project to CNN dimension
        x = self.projection(x)  # [B, N, d_model_vit] -> [B, N, d_model_cnn]
    ...
    def get_intermediate_layers(self, x, mask=None, n=1):
        ...
        Returns:
            List of [B, N, d_model_cnn] tensors from specified layers
        """
        layer_outputs = []
        ...
        # Project each layer output to CNN dimension
        for layer_out in selected_layers:
            projected = self.projection(layer_out)
            if self.use_layernorm:
